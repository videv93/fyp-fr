...
class SkepticAgent:
    ...
    def audit_vulnerabilities(
        self, contract_source: str, vulnerabilities: list
    ) -> list:
        """
        Takes the full contract source code plus the list of all vulnerabilities
        and produces a confidence ranking for each vulnerability.

        Returns the updated list of vulnerabilities, each with a `skeptic_confidence`
        and sorted descending by `skeptic_confidence`.
        """
        if not vulnerabilities:
            return []

        # We'll pass them all in a single prompt. (You can also chunk them if there's a lot.)
...
"""
        ...
        print("SkepticAgent response:", text_out)

        # Parse the JSON
        try:
            data = json.loads(text_out)
            rechecked = data.get("rechecked_vulnerabilities", [])
        except json.JSONDecodeError:
            print("Error parsing JSON from SkepticAgent")
            print("Trying to extract JSON block from text...")
            # This is due to the triple backticks and json formatting
        ...
        # Now merge it back into the original vulnerabilities list
        # and store the skeptic_confidence + validity_reasoning
        ...
        # Sort them by skeptic_confidence desc
        vulnerabilities_sorted = sorted(
            vulnerabilities, key=lambda x: x.get("skeptic_confidence", 0), reverse=True
        )
        return vulnerabilities_sorted
