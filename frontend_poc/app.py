...
def start_analysis():
    ...
    threading.Thread(
        ...
        args=(job_id, job['contract_path'], model_config, auto_run_config)
    ).start()
...
def analyze_thread(job_id, contract_path, model_config, auto_run_config):
    try:
        ...
        coordinator = SocketIOAgentCoordinator(job_id, model_config=model_config)
...
class SocketIOAgentCoordinator(AgentCoordinator):
    def __init__(self, job_id, model_config=None):
    ...
    def analyze_contract(self, contract_info, auto_run_config=None):
        ...
        if high_conf_vulns:
            for i, vul in enumerate(high_conf_vulns):
                ...
                # First generate the basetest.sol file if it doesn't exist
                ...
                if not os.path.exists("exploit/src/test/basetest.sol"):
                    ...
                    print(f"Created base file: {base_file}")
