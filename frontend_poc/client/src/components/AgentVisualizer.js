...
const AgentVisualizer = ({ activeAgent, status, completedAgents = [] }) => {
  ...
  const workflowSteps = [
    { id: 'contract', label: 'Smart Contract', description: 'Input smart contract ready for analysis' },
    { id: 'static_analyzer', label: 'Static Analyzer', description: 'Running static analysis tools' },
    { id: 'analyzer', label: 'Analyzer Agent', description: 'Detecting potential vulnerabilities' },
    { id: 'skeptic', label: 'Skeptic Agent', description: 'Verifying vulnerability validity' },
    { id: 'exploiter', label: 'Exploiter Agent', description: 'Creating exploit plans' },
    { id: 'generator', label: 'Generator Agent', description: 'Generating proof-of-concept code' },
    { id: 'runner', label: 'Runner Agent', description: 'Testing and fixing exploits' },
    { id: 'results', label: 'Analysis Results', description: 'Final vulnerability analysis completed' }
  ];
  ...
  const getCurrentStepIndex = () => {
    ...
    if (activeAgent) {
      const activeIndex = workflowSteps.findIndex(step => step.id === activeAgent);
    ...
    <div className="bg-white p-6 rounded-lg shadow-md">
        ...
        <div className="space-y-4">
          {workflowSteps.map((step, index) => {
            ...
            return (
              <div 
                key={step.id} 
                className={`flex items-center p-3 border rounded-md ${bgColor} transition-all duration-300`}
              >
                <div className={`w-8 h-8 rounded-full flex items-center justify-center mr-3 ${iconColor}`}>
                  {stepStatus === 'completed' ? (
        ...
        <div className="mt-6 p-3 bg-blue-50 border border-blue-200 rounded-md flex items-center">
          ...
          <div>
            ...
            <span className="text-blue-600">
              {activeAgent === 'static_analyzer' ? 'Running static analysis' : 
               activeAgent === 'analyzer' ? 'Analyzing contract for vulnerabilities' :
               activeAgent === 'skeptic' ? 'Verifying vulnerability validity' :
               activeAgent === 'exploiter' ? 'Creating exploit plan' :
               activeAgent === 'generator' ? 'Generating PoC code' :
               activeAgent === 'runner' ? 'Running and fixing exploit' :
               'Processing...'}
            </span>
          </div>
        </div>
      ...
      {status === 'completed' && (
        <div className="mt-6 p-3 bg-green-50 border border-green-200 rounded-md flex items-center">
          ...
          <span className="font-medium text-green-700">Analysis completed successfully</span>
        </div>
      )}
