...
const VulnerabilitiesPanel = ({ vulnerabilities }) => {
  ...
  const selectedVuln = selectedVulnIndex !== null ? vulnerabilities[selectedVulnIndex] : null;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-1 border rounded-md overflow-hidden">
          <div className="bg-gray-50 px-4 py-2 border-b">
            <h3 className="font-medium">Vulnerabilities ({vulnerabilities.length})</h3>
          </div>
        ...
        <div className="lg:col-span-2 border rounded-md">
          {selectedVuln ? (
            <div>
              <div className="bg-gray-50 px-4 py-2 border-b">
                <h3 className="font-medium">{selectedVuln.vulnerability_type}</h3>
              </div>
              <div className="p-4 space-y-4 max-h-96 overflow-y-auto">
                <div>
                  ...
                  <div className="w-full bg-gray-200 rounded-full h-2.5">
                    <div 
                      ...
                      style={{ width: `${(selectedVuln.skeptic_confidence || 0) * 100}%` }}
                    ></div>
                  </div>
                ...
                <div>
                  <h4 className="text-sm font-medium text-gray-700">Validity Assessment</h4>
                  <p className="text-sm text-gray-600 mt-1">
                    {selectedVuln.validity_reasoning || 'No validity assessment provided'}
                  </p>
                </div>
                
                <div>
                  <h4 className="text-sm font-medium text-gray-700">Affected Functions</h4>
                  {selectedVuln.affected_functions && selectedVuln.affected_functions.length > 0 ? (
                    ...
                    <p className="text-sm text-gray-500 mt-1">No specific functions identified</p>
                  )}
                </div>
                
                {selectedVuln.code_snippet && (
                  <div>
                    <h4 className="text-sm font-medium text-gray-700">Code Snippet</h4>
                ...
                {selectedVuln.exploitation_scenario && (
                  <div>
                    <h4 className="text-sm font-medium text-gray-700">Exploitation Scenario</h4>
                    <p className="text-sm text-gray-600 mt-1">
                      {selectedVuln.exploitation_scenario}
                    </p>
                  </div>
